...
🗂️ Creates and organizes compressed archives (.tar.gz)
...
Defines your project source folder and backup destination.

Creates a new .tar.gz archive using the current date and time.

Stores the compressed backup inside a dedicated backups folder.

Logs each operation (success or failure) in backup.log.

Can be automated using a cron job for daily or weekly backups.
...
![Backup Script Execution](./images/backup_run.png)

🧾 Log File Example

![Backup Folder](./images/backup_folder.png)
